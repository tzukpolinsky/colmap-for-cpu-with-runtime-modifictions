...
@image html sift-frame.png "SIFT keypoints are circular image regions with an
orientation."
...
@image html sift-descr-easy.png "The SIFT descriptor is a spatial histogram of
the image gradient."
...
- For each octave in the scale space:
  ...
  - For each keypoint:
    - Use ::vl_sift_calc_keypoint_orientations() to get the keypoint
orientation(s).
...
The canonized image at scale @f$ \hat \sigma @f$ is in relation with the scaled
image
...
Therefore we can compute the descriptor either in the image or canonical frame
as:
...
static void copy_and_upsample_rows(vl_sift_pix* dst, vl_sift_pix const* src,
                                   int width, int height) {
  int x, y;
  vl_sift_pix a, b;
  int dstAdvance = 1 - width * 2 * height;
  for (y = 0; y < height; ++y) {
    a = *src++;
    for (x = 0; x < width - 1; ++x) {
      b = *src++;
...
static void _vl_sift_prepare_gaussFilter(VlSiftFilt* self, double sigma) {
  vl_uindex j;
  vl_sift_pix acc = 0;
  ...
  self->gaussFilter =
      vl_malloc(sizeof(vl_sift_pix) * (2 * self->gaussFilterWidth + 1));

  for (j = 0; j < 2 * self->gaussFilterWidth + 1; ++j) {
    vl_sift_pix d =
        ((vl_sift_pix)((signed)j - (signed)self->gaussFilterWidth)) /
        ((vl_sift_pix)sigma);
...
static void _vl_sift_smooth(VlSiftFilt* self, vl_sift_pix* outputImage,
                            vl_sift_pix* tempImage,
                            vl_sift_pix const* inputImage, vl_size width,
                            vl_size height, double sigma) {
  /* prepare Gaussian filter */
  if (self->gaussFilterSigma != sigma) {
    _vl_sift_prepare_gaussFilter(self,sigma);
  }
  ...
  vl_imconvcol_vf(tempImage, height, inputImage, width, height, width,
                  self->gaussFilter, -self->gaussFilterWidth,
                  self->gaussFilterWidth, 1,
                  VL_PAD_BY_CONTINUITY | VL_TRANSPOSE);

  vl_imconvcol_vf(outputImage, width, tempImage, height, width, height,
                  self->gaussFilter, -self->gaussFilterWidth,
                  self->gaussFilterWidth, 1,
                  VL_PAD_BY_CONTINUITY | VL_TRANSPOSE);
}
...
static void copy_and_downsample(vl_sift_pix* dst, vl_sift_pix const* src,
...
VlSiftFilt* vl_sift_new(int width, int height, int noctaves, int nlevels,
  ...
  f->octave = vl_malloc(sizeof(vl_sift_pix) * nel * (f->s_max - f->s_min + 1));
  f->dog = vl_malloc(sizeof(vl_sift_pix) * nel * (f->s_max - f->s_min));
  f->grad = vl_malloc(sizeof(vl_sift_pix) * nel * 2 * (f->s_max - f->s_min));
...
int vl_sift_process_first_octave(VlSiftFilt* f, vl_sift_pix const* im) {
  ...
  if (o_min < 0) {
    ...
    for (o = -1; o > o_min; --o) {
      ...
      copy_and_upsample_rows(octave, temp, width << -o, 2 * (height << -o));
    }
  } else if (o_min > 0) {
    /* downsample */
...
int vl_sift_process_next_octave(VlSiftFilt* f) {
  ...
  if (f->o_cur == o_min + O - 1) return VL_ERR_EOF;
...
#define CHECK_NEIGHBORS(CMP, SGN)                                           \
  (v CMP## = SGN 0.8 * tp && v CMP * (pt + xo) && v CMP * (pt - xo) &&      \
             v CMP * (pt + so) && v CMP * (pt - so) && v CMP * (pt + yo) && \
             v CMP * (pt - yo) &&                                           \
                                                                            \
        ...
        if (CHECK_NEIGHBORS(>, +) || CHECK_NEIGHBORS(<, -)) {
          ...
          if (f->nkeys >= f->keys_res) {
            ...
            if (f->keys) {
              f->keys =
                  vl_realloc(f->keys, f->keys_res * sizeof(VlSiftKeypoint));
            } else {
              f->keys = vl_malloc(f->keys_res * sizeof(VlSiftKeypoint));
            }
          }
  ...
  for (i = 0; i < f->nkeys; ++i) {
    ...
    double Dx = 0, Dy = 0, Ds = 0, Dxx = 0, Dyy = 0, Dss = 0, Dxy = 0, Dxs = 0,
           Dys = 0;
    double A[3 * 3], b[3];
...
#define Aat(i, j) (A[(i) + (j)*3])
    ...
    /* check threshold and other conditions */
    {
      ...
      vl_bool good = vl_abs_d(val) > tp && score < (te + 1) * (te + 1) / te &&
                     score >= 0 && vl_abs_d(b[0]) < 1.5 &&
                     vl_abs_d(b[1]) < 1.5 && vl_abs_d(b[2]) < 1.5 && xn >= 0 &&
                     xn <= w - 1 && yn >= 0 && yn <= h - 1 && sn >= s_min &&
                     sn <= s_max;
...
int vl_sift_calc_keypoint_orientations(VlSiftFilt* f, double angles[4],
  ...
  if (k->o != f->o_cur) return 0;

  /* skip the keypoint if it is out of bounds */
  if (xi < 0 || xi > w - 1 || yi < 0 || yi > h - 1 || si < f->s_min + 1 ||
      si > f->s_max - 2) {
    return 0;
  }
...
#define at(dx, dy) (*(pt + xo * (dx) + yo * (dy)))
  ...
  for (ys = VL_MAX(-W, -yi); ys <= VL_MIN(+W, h - 1 - yi); ++ys) {
    for (xs = VL_MAX(-W, -xi); xs <= VL_MIN(+W, w - 1 - xi); ++xs) {
  ...
  maxh = 0;
  for (i = 0; i < nbins; ++i) maxh = VL_MAX(maxh, hist[i]);
  ...
  for (i = 0; i < nbins; ++i) {
    ...
    if (h0 > 0.8 * maxh && h0 > hm && h0 > hp) {
      ...
      angles[nangles++] = th;
      if (nangles == 4) goto enough_angles;
    }
  }
enough_angles:
  return nangles;
...
VL_INLINE vl_sift_pix normalize_histogram(vl_sift_pix* begin,
  ...
  for (iter = begin; iter != end; ++iter) norm += (*iter) * (*iter);

  norm = vl_fast_sqrt_f(norm) + VL_EPSILON_F;

  for (iter = begin; iter != end; ++iter) *iter /= norm;
...
VL_EXPORT
void vl_sift_calc_raw_descriptor(VlSiftFilt const* f, vl_sift_pix const* grad,
  ...
  int const W = floor(sqrt(2.0) * SBP * (NBP + 1) / 2.0 + 0.5);
...
#define atd(dbinx, dbiny, dbint) \
  *(dpt + (dbint)*binto + (dbiny)*binyo + (dbinx)*binxo)
   ...
   */
  for (dyi = VL_MAX(-W, -yi); dyi <= VL_MIN(+W, h - yi - 1); ++dyi) {
    for (dxi = VL_MAX(-W, -xi); dxi <= VL_MIN(+W, w - xi - 1); ++dxi) {
      /* retrieve */
      ...
      vl_sift_pix win =
          fast_expn((nx * nx + ny * ny) / (2.0 * wsigma * wsigma));
        ...
        for (dbiny = 0; dbiny < 2; ++dbiny) {
          for (dbint = 0; dbint < 2; ++dbint) {
            if (binx + dbinx >= -(NBP / 2) && binx + dbinx < (NBP / 2) &&
              ...
              vl_sift_pix weight = win * mod * vl_abs_f(1 - dbinx - rbinx) *
  ...
  if (1) {
     ...
     */
    int numSamples = (VL_MIN(W, w - xi - 1) - VL_MAX(-W, -xi) + 1) *
                     (VL_MIN(W, h - yi - 1) - VL_MAX(-W, -yi) + 1);

    if (f->norm_thresh && norm < f->norm_thresh * numSamples) {
      for (bin = 0; bin < NBO * NBP * NBP; ++bin) descr[bin] = 0;
    } else {
      /* truncate at 0.2. */
...
void vl_sift_calc_keypoint_descriptor(VlSiftFilt* f, vl_sift_pix* descr,
                                      VlSiftKeypoint const* k, double angle0) {
  /*
  ...
  */
  ...
  /* check bounds */
  if (k->o != f->o_cur || xi < 0 || xi >= w || yi < 0 || yi >= h - 1 ||
...
#define atd(dbinx, dbiny, dbint) \
  *(dpt + (dbint)*binto + (dbiny)*binyo + (dbinx)*binxo)
   ...
   * (1,1)-(M-1,N-1) and the keypoint bounding box.
   */
  for (dyi = VL_MAX(-W, 1 - yi); dyi <= VL_MIN(+W, h - yi - 2); ++dyi) {
    for (dxi = VL_MAX(-W, 1 - xi); dxi <= VL_MIN(+W, w - xi - 2); ++dxi) {
      /* retrieve */
      vl_sift_pix mod = *(pt + dxi * xo + dyi * yo + 0);
      vl_sift_pix angle = *(pt + dxi * xo + dyi * yo + 1);
      vl_sift_pix theta = vl_mod_2pi_f(angle - angle0);
...
VL_EXPORT
void vl_sift_keypoint_init(VlSiftFilt const* f, VlSiftKeypoint* k, double x,
